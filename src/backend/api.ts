...
export default async (bp: typeof sdk) => {
   ...
   * Your API will be available at `http://localhost:3000/api/v1/bots/BOT_NAME/mod/bp-report`
   * Just replace BOT_NAME by your bot ID
   */
  const router = bp.http.createRouterForBot('bp-report')

  // Link to access this route: http://localhost:3000/api/v1/bots/BOT_NAME/mod/bp-report/my-first-route
  router.get('/my-first-route', async (req, res) => {
     ...
     */
    const config = await bp.config.getModuleConfigForBot('bp-report', botId)
